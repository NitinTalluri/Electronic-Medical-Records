...
def userSignup(user_id,user_is):
    ...
    query = "INSERT into auth (username ,userpassword ,user_is, user_id) values (%s,%s,%s,%s)"
    values = (username,password,user_is,user_id)
...
def setUserId(userid):
    connection = create_db_connection()
    cursor = connection.cursor()
    cursor.callproc('my_proc',(user_id,))

    # get the value of the variable uid
